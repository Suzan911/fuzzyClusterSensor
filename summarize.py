...
def get_overall(density, size, t_init, is_fuzzy=True):
    path = config.root + ("%.5f" % density) + ("_fuzzy" if is_fuzzy else "_fixed")
    try:
        data = xl.load_workbook(path + "/R%02d/R%02dT%ddata.xlsx" % (size, size, t_init))
...
if __name__ == "__main__":
    ...
    for density in config.density:
        ...
        plt.plot(config.size, sop_9, label='T0.9', color="magenta")
        plt.xlabel('Size')
        plt.ylabel('Round')
        ...
        plt.plot(config.size, size_9, label='T0.9', color="magenta")
        ...
        plt.plot(config.size, t_9, label='T0.9', color="magenta")
        plt.xlabel('Size')
        plt.ylabel('T')
